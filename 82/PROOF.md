...
The whole support-`<=2` layer is in fact polynomially bounded by a separator
argument.
...
Proof.  Let `M` be the multiset of trace vectors.  Since no nonempty
submultiset of `M` has sum zero, the origin is not in the nonnegative real
cone generated by `M`: otherwise a nonzero real dependence
`sum c_b v_b=0` with `c_b>=0` could be replaced by a rational one, because
the defining equations have integer coefficients, and clearing denominators
would give a forbidden integer submultiset.  By Gordan's alternative, there is
a real vector `x in R^d` such that

```text
x dot v_b > 0       for every b in B.
```

Perturb `x` if necessary so that no coordinate is zero and no sum
`x_i+x_j` is zero; all displayed strict inequalities remain true.
...
For support-one traces, `+e_i` forces `i in P` and `-e_i` forces `i in N`.
For support-two traces:

* `+1_{i,j}` cannot have both endpoints in `N`;
* `-1_{i,j}` cannot have both endpoints in `P`;
* if `i in P` and `j in N`, then `+1_{i,j}` means `|x_i|>|x_j|`, while
  `-1_{i,j}` means `|x_i|<|x_j|`.

Build a directed comparison graph on the coordinate set as follows.  For a
support-two trace with one endpoint `p in P` and the other endpoint `n in N`,
add the arc `n->p` for a positive trace and the arc `p->n` for a negative
trace.  The inequalities above show that this directed graph is acyclic.
Let `ell(i)` be one plus the maximum length of a directed path ending at `i`;
then `1<=ell(i)<=d`.

Define an integer vector `a` by
...
We claim that `a dot v_b >=1` for every trace vector.  This is immediate for
singletons.  A positive pair inside `P` has positive dot product, and a
negative pair inside `N` also has positive dot product.  The forbidden
same-sign cases do not occur.  For a positive pair `p,n` with `p in P` and
`n in N`, the arc `n->p` gives `ell(p)>ell(n)`, so
`a dot 1_{p,n}=ell(p)-ell(n)>=1`.  The negative cross-sign case is identical,
using the arc `p->n`.

Thus `a` is an integer separating functional with
