...
def residues_for_assignment(
    ...
    for vertex, color in enumerate(assignment):
        residue = (adj[vertex] & subsets[color]).bit_count() % modulus
...
def score_assignment(
    ...
    score = score_residues(
        residues_for_assignment(n, modulus, assignment, colors, adj),
        modulus,
    )
    if max_part_size is not None:
        ...
        for color in assignment:
            sizes[color] += 1
        score += sum(max(0, size - max_part_size) for size in sizes)
    return score
...
def search(
    ...
    for restart in range(restarts):
        ...
        current = score_assignment(
            n, modulus, assignment, colors, adj, max_part_size
        )
...
def sample_full_modular(
    ...
    while accepted < trials and attempts < max_attempts:
        attempts += 1
        if full_modulus == 1:
            graph_mask = rng.getrandbits(edge_count_value)
        else:
            assert pc is not None
            graph_mask = modular_partition.random_full_modular_candidate(
                n, full_modulus, edge_count_value, rng, pc, edge_index
            )
...
def assignment_stats(
    ...
    for color in colors:
        ...
        if subset == 0:
            continue
        vertices = [
            vertex for vertex, assigned in enumerate(assignment) if assigned == color
        ]
...
def main() -> None:
    ...
    if args.max_part_size is not None:
