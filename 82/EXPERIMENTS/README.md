...
- `defect_structure_scan.py`: structural scanner for connected graphs with
  spectrum-mass defect.  It reports cut vertices, first two-cuts, simplicial
  degree-two vertices, and whether non-cut vertex deletions have full mass.
...
- `feedback_extension_diagnostic.py`: for a fixed graph with a
  ...
  how the extension fills the one-unit spectrum-mass deficit; it also has an
  exact `--scan` mode for small labelled graphs and a randomized `--sample`
  mode for larger graphs.
...
- `source_slots_fast.cpp`: C++ exact checker for fixed source-residue dyadic
  ...
  minimal-counterexample regimes.  Use `--flexible-parts B` to ignore fixed
  residue signatures and test whether each graph has some partition into at
  most `B` target-modular parts.
...
- `degree_bucket_profile.py`: groups vertices of a fixed mask by exact global
  ...
  adjacency/shifted ranks, both in the graph and in the complement.  It also
  reports the maximum pairwise row-difference diameter inside each bucket,
  split into global, internal, and external traces.  This calibrates
  Corollary 7D, Corollary 28D.3, and the degree-bucket reductions around
  Proposition 28F.
...
- `column_drop_census.py`: exact small labelled census for the column-drop
  ...
  induced witnesses.  It also has `--search-h` for depth-first construction of
  lower examples with no homogeneous set of the requested order,
  `--search-regular-h` for examples with no regular induced subgraph of at
  least the requested order, and fixed-mask inspection support.
