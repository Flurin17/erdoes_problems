...
- `n = 63` and `n = 99` remain open in this workspace: all other source rows
  ...
  `5867040` shells. All word groups are now representative-classified with up
  ...
  mode; replaying the first `1000000` profile groups checks all `2093554`
  outside-cover realizations in those groups with no mixed-status group and no
  count mismatch. The exact prefix split is `1047039` corner-label violations,
  `638288` pinch-sector obstructions, and `408227` split-corner-label
  obstructions.
