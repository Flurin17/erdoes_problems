...
def gamma_2alpha_status(n: int) -> str:
    ...
    if n in {63, 99}:
        return "ruled out by boundary fan plus side-difference nonfit obstruction"
...
def workspace_negative_reason(n: int) -> str | None:
    ...
    if n in {38, 39, 42}:
        return "negative by the 38/39/42 composite benchmark in PROOF.md"
    if n in {46, 51}:
    ...
    if n in {63, 99}:
        return "negative by the 63/99 gamma=2alpha boundary nonfit benchmark in PROOF.md"
    if n in {76, 92}:
    ...
    if n in {78, 86, 87, 88, 91, 93, 94, 95}:
        return "negative by the 78/86/87/88/91/93/94/95 composite benchmark in PROOF.md"
    if n == 22:
