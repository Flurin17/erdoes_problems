...
## `N=21` and `N=30` Exact Filter Certificate

The next two dashboard-empty composites were rerun with exact equilateral
arithmetic:

```sh
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/equilateral_boundary_exact.py 21 30
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/equilateral_pi_boundary.py 21 16 21
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/equilateral_pi_boundary.py 30 8 15
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/beeson_3alpha2beta_filter.py 21 30 --json
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/beeson_isosceles_alpha_plus_beta_filter.py 21 30 --json
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/gamma_2pi3_isosceles_filter.py 21 30
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/gamma_2pi3_nonisosceles_exact.py 21 30
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/gamma_2pi3_nonisosceles_boundary.py
```

The exact equilateral scan leaves only `pi/3` candidates:

```text
N=21: (16,21,19), L=84 and its a/b swap
N=30: (8,15,13), L=60 and its a/b swap
```

Both fail the `pi/3` equilateral boundary-star check with two oriented side
paths and zero compatible full boundary cycles.

The `3alpha+2beta=pi` first-pass filter gives five raw roots for each count.
For `21`, four isosceles-`alpha+beta` roots fail Beeson's stronger Section 11.4
filter and the remaining `(2,3,4)` isosceles-alpha root is removed by the
boundary-star check. For `30`, all five raw roots are isosceles-`alpha+beta`
and fail the stronger Section 11.4 filter. The `gamma=2pi/3` isosceles filter
returns zero candidates for both counts.

The exact non-isosceles `gamma=2pi/3` filter leaves one candidate for each
count, `(5,16,19)` at scale `4` for `21` and `(7,8,13)` at scale `4` for `30`;
the endpoint boundary-star script reports zero feasible full boundary endpoint
cycles for both. Therefore `PROOF.md` treats `21` and `30` as classified
negative values.
...
## `N=33` and `N=35` Exact Filter Certificate

The next two benchmark values were checked in a small exact batch:

```sh
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/equilateral_boundary_exact.py 33
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/equilateral_boundary_exact.py 35
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/composite_case_dashboard.py 33 35 --equilateral-exact
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/beeson_3alpha2beta_filter.py 33 35 --json
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/gamma_2pi3_isosceles_filter.py 33 35
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/gamma_2pi3_nonisosceles_exact.py 33 35
```

Both counts have zero exact equilateral boundary-length candidates and zero
exact non-isosceles `gamma=2pi/3` candidates. The isosceles `gamma=2pi/3`
filter gives no candidate for `35`; for `33`, it gives only
`(a,b,c)=(5,3,7)`, `m=3`, `X=21`, `Y=33`, which is removed by the global
boundary-transition lemma for this source row.

The `3alpha+2beta=pi` first-pass filter gives five raw
isosceles-`alpha+beta` roots for each count, and Beeson's stronger Section 11.4
...
## `N=46` and `N=51` Exact Filter Certificate

The next pair was checked with exact equilateral arithmetic:

```sh
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/composite_case_dashboard.py 46 51 --equilateral-exact
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/equilateral_boundary_exact.py 46 51
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/beeson_3alpha2beta_filter.py 46 51 --json
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/beeson_isosceles_alpha_plus_beta_filter.py 46 51 --json
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/gamma_2pi3_isosceles_filter.py 46 51
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/gamma_2pi3_nonisosceles_exact.py 46 51
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/beeson_3alpha2beta_boundary.py
```

Both counts have zero exact equilateral candidates and zero exact
non-isosceles `gamma=2pi/3` candidates. The isosceles `gamma=2pi/3` filter
returns no candidate for `51`; for `46`, it returns only `(7,8,13)`, `m=4`,
`X=52`, `Y=92`, which is removed by the boundary-transition lemma.

For `46`, six `3alpha+2beta` raw roots fail Beeson's stronger Section 11.4
filter, and the remaining triquadratic root `(10,21,25)` has zero full
boundary cycles in `beeson_3alpha2beta_boundary.py`. For `51`, all seven raw
roots fail the stronger Section 11.4 filter. Therefore `PROOF.md` treats `46`
and `51` as classified negative values.

## `N=55` Exact Filter Certificate

The next value was checked separately because it has one non-isosceles
`gamma=2pi/3` candidate:

```sh
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/equilateral_boundary_exact.py 55
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/equilateral_pi_boundary.py 55 16 55
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/beeson_3alpha2beta_filter.py 55 --json
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/beeson_isosceles_alpha_plus_beta_filter.py 55 --json
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/gamma_2pi3_isosceles_filter.py 55
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/gamma_2pi3_nonisosceles_exact.py 55
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/gamma_2pi3_nonisosceles_boundary.py
```

The exact equilateral scan leaves only `(16,55,49)`, `L=220`, and its `a,b`
swap in the `pi/3` model. The representative boundary-star check has two
oriented side paths and zero compatible full boundary cycles.

The `3alpha+2beta=pi` first-pass filter gives seven raw
isosceles-`alpha+beta` roots, all rejected by Beeson's stronger Section 11.4
filter. The isosceles `gamma=2pi/3` filter returns zero candidates. The exact
non-isosceles `gamma=2pi/3` filter leaves one candidate, `(39,16,49)` at scale
`4`, and the endpoint boundary-star check reports zero feasible full boundary
endpoint cycles. Therefore `PROOF.md` treats `55` as a classified negative
value.

## `N=57` and `N=62` Exact Filter Certificate

The next two squarefree composite no-survivor rows were rerun exactly:

```sh
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/equilateral_boundary_exact.py 57
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/equilateral_boundary_exact.py 62
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/beeson_3alpha2beta_filter.py 57 62 --json
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/beeson_isosceles_alpha_plus_beta_filter.py 57 62 --json
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/gamma_2pi3_isosceles_filter.py 57 62
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/gamma_2pi3_nonisosceles_exact.py 57 62
PYTHONDONTWRITEBYTECODE=1 python3 -B 634/EXPERIMENTS/composite_case_dashboard.py 57 62 --equilateral-side-bound 250
```

The exact equilateral scan returns zero boundary-length candidates for both
values. The `gamma=2pi/3` isosceles and non-isosceles filters also return zero
candidates. In the `3alpha+2beta=pi` branch, `57` has seven raw rational roots
and `62` has eight. The stronger isosceles-`alpha+beta` filter rejects all
seven roots for `57` and seven of the eight roots for `62`; the remaining
...
values by local certificates.

The local script leaves one `N=60` `gamma=2alpha` candidate:
