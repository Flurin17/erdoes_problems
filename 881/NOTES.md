...
Corollary 3.4d.20 splits the active pair-packet obstruction into a
singleton unique-gate branch and a nonsingleton branch with parallel
retained copies whose self-shadows and cross-shadows are trapped in one
bounded palette.
Warnings 3.4d.21--3.4d.22 record scalable finite cautions using thickened
dilations. For every \(p\ge1\), the union of intervals
\(\bigcup_{a\in\{1,2,3,4,7\}}[ap,ap+p-1]\) covers \([8p,16p]\) by three
sums but has a four-point packet \(p\{1,3,4,7\}\) whose anchored shadows
through \(2p\) all have size at most \(2\). Likewise the thickened
\(\{1,2,3,4,5,7\}\) dilation contains parallel copies \(p\{1,3,5\}\) and
\(p\{3,5,7\}\) whose self-shadows and cross-shadows are trapped in
...
menus cannot hide in countably many active labels. Corollary 16.136 ties
...
only into \(2A\). A proof must still convert those mirrors into four-term
repairs, force unbounded depth/rank/profile height, or rule out the
long-range far-gate geometry.
...
the modulo-\(10\) booster restriction, default pair stages can reach depth
four:
\[
...
three-point increments up to \(120\) finds no next stage. A wider
`--depth5` run finds a nonperiodic chain
\[
\{1,2,3\}\to\{1,2,3,7\}\to\{1,2,3,7,10\}\to
\{1,2,3,7,10,23\}\to
\{1,2,3,7,10,23,36\}\to
\{1,2,3,7,10,23,36,46\},
\]
...
certificate.
Lemma 8.5a.7z.12h.1 identifies the one-point part as external translate
absorption: for candidates \(x=p-a\), the condition \(w-x\in2C\) is exactly
\(d+a\in2C\). The endpoint \(3000\) shadow summary has \(d=3100\notin C\),
\(d\in2C\), and \(2311\) absorbed old rows, \(2308\) retained and \(3\)
deleted.
Corollary 8.5a.7z.12h.2 gives the escape-set form. At endpoint \(3000\)
there are no old-row escapes; the split escape set has \(2312\) points but
no complementary pair summing to \(6900\), and the half-candidate is
saturated.
Warning 8.5a.7z.12h.3 notes that this cannot be beaten by cardinality:
