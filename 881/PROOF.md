...
### Lemma 8.5a.7u': Tail product mass is necessary for selector barriers

Let
\[
F_1,F_2,\ldots
\]
be finite nonempty blocks. A finite cylinder is a pair
\[
\mathcal C=(J,(Y_j)_{j\in J}),
\]
where \(J\subset\mathbb N\) is finite nonempty and
\[
\varnothing\ne Y_j\subseteq F_j\qquad(j\in J).
\]
It catches a selector \(x_j\in F_j\) on a tail \(I\subset\mathbb N\) if
\[
J\subset I,\qquad x_j\in Y_j\quad(j\in J).
\]
Define its product mass by
\[
\mu(\mathcal C)=\prod_{j\in J}{|Y_j|\over |F_j|}. \tag{1}
\]
Let \(\mathscr C\) be a countable family of finite cylinders. If, for some
\(t\),
\[
\sum_{\substack{\mathcal C\in\mathscr C\\ J(\mathcal C)\subset[t,\infty)}}
\mu(\mathcal C)<1, \tag{2}
\]
then there is a selector
\[
x_j\in F_j\qquad(j\ge t)
\]
which is caught by none of the cylinders whose supports lie in
\([t,\infty)\).

Consequently, if a family of arithmetic bad edges is a product-selector
barrier on every tail, and containment of any such edge in a selector tail
forces that selector to be caught by a cylinder from \(\mathscr C\), then
the tail sum in (2) is at least \(1\) for every \(t\).

Proof. Choose a selector on the tail \(j\ge t\) by taking the coordinates
independently and uniformly from \(F_j\). The probability that a fixed
cylinder \(\mathcal C\) with support in the tail catches the selector is
exactly \(\mu(\mathcal C)\). By the union bound, the probability that some
tail cylinder catches the selector is at most the sum in (2), which is
less than \(1\). Hence a selector avoiding all those cylinders exists.

For the consequence, if the tail sum were less than \(1\) on some tail, the
selector just constructed would avoid every cylinder there, and therefore
could not contain any bad edge whose containment is witnessed by those
cylinders. This contradicts the product-selector barrier property on that
tail. \(\square\)

This is the infinite-tail version of the finite product count in Lemma
8.5a.7u. It is deliberately only a mass criterion: abstract Schreier
barriers can evade it by having nonsummable tail mass. Its use later is to
separate genuinely large product debt from low-density packet artefacts.
If interval geometry shows that all possible bad packets lie in cylinders
...
### Corollary 16.86: Robust core families cover bounded palettes
...
Let \(F\subset A\) satisfy \(1\le |F|\le r\), let \(P\subset F\), and for
each \(f\in P\) let \(U_f\subset I\) satisfy the hypotheses of Corollary
16.45 for the active color \(f\), with witness parameter sufficiently large
for the fixed interval \(I\) and rank bound \(r\). Let
\[
\mathcal K
\]
be a finite family of auxiliary intervals
\[
K=[c_K,d_K]\cap\mathbb N\subset A.
\]
For each \(K\), put
\[
m_K=d_K-c_K+1,\qquad
\ell_K=\left\lceil {m_K-r\over r+1}\right\rceil,
\qquad
M_\delta=\lfloor(1-\delta)n\rfloor+1,
\]
and assume
\[
M_\delta\le2\ell_K-1. \tag{2}
\]
If
\[
P\subseteq\bigcup_{K\in\mathcal K}R_K(\delta,I;r), \tag{3}
\]
then
\[
\left|\bigcup_{f\in P}U_f\right|<\eta n. \tag{4}
...
Proof. For every \(K\in\mathcal K\), Lemma 16.52 applied to \(F\cap K\)
inside \(K\) gives a retained interval
\[
J_K\subset K\cap(A\setminus F)
\]
of length at least \(\ell_K\). As in Lemma 16.69,
\[
R_K(\delta,I;r)\subseteq W_{J_K}(\delta,I). \tag{5}
\]
Thus (3) implies that \(P\) is contained in the union of the actual blocker
windows
\[
\bigcup_{K\in\mathcal K}W_{J_K}(\delta,I).
\]
Corollary 16.74, applied to this finite retained-interval family, proves
(4). \(\square\)

Thus a dense bounded-rank interval packet can only use a finite active
palette that escapes every finite robust-core cover generated by auxiliary
intervals. This is the deletion-pattern-free form of the finite
blocker-window obstruction.

### Corollary 16.87: Vanishing packet debt must be nonsummable

Work in the \(k=3\) nonsingleton-front setting of Corollary 16.35. Suppose
a tail of the front has been organized into finite disjoint active-color
packets
\[
P_1,P_2,\ldots\subset A,
\]
and consider selectors \(x_s\in P_s\). Fix \(r\ge1\) and
\(0<\eta\le1\). Let \(\mathcal G_{r,\eta}\) be any family of active traces
\[
G\subset\bigcup_sP_s
\]
with the following properties:

1. each \(G\) meets each packet \(P_s\) in at most one point and
   \(|G|\le r\);
2. each \(G\) has an order-\(4\) witness and an \(\eta\)-dense bounded-rank
   interval packet satisfying the hypotheses of Corollary 16.45;
3. \(\mathcal G_{r,\eta}\) product-covers every selector tail: for every
   infinite \(I\subset\mathbb N\) and every selector
   \(x_s\in P_s\) for \(s\in I\), there is
   \(G\in\mathcal G_{r,\eta}\) with
   \(G\subset\{x_s:s\in I\}\).

Let \(\mathscr C_{r,\eta}\) be a countable family of finite product
cylinders on the packets \(P_s\) such that whenever a selector contains an
edge \(G\in\mathcal G_{r,\eta}\), that selector is caught by at least one
cylinder in \(\mathscr C_{r,\eta}\). Then, for every tail index \(t\),
\[
\sum_{\substack{\mathcal C\in\mathscr C_{r,\eta}\\
J(\mathcal C)\subset[t,\infty)}}\mu(\mathcal C)\ge1. \tag{1}
\]
Equivalently, if the cylinder masses for this branch are summable on some
tail with total \(<1\), then there is a selector tail which avoids every
bounded-rank \(\eta\)-dense interval-packet trace in
\(\mathcal G_{r,\eta}\).

Proof. Apply Lemma 8.5a.7u' to the packet family \(P_s\) and the cylinder
family \(\mathscr C_{r,\eta}\). If (1) failed on a tail, that lemma would
produce a selector on the tail avoiding all cylinders. By the cylinder
capture hypothesis, this selector would contain no member of
\(\mathcal G_{r,\eta}\), contradicting the asserted product-selector
barrier property of the bounded-rank \(\eta\)-dense branch. \(\square\)

In applications the cylinders come from interval geometry. For example,
Corollary 16.86 says that an \(\eta\)-dense bounded-rank packet cannot use
an active palette covered by robust cores; hence a packetized construction
must place selected active colors in the complementary escape sets. If the
relative sizes of those escape sets have summable product mass on a tail,
the whole bounded-rank positive-density branch is selector-avoidable. Thus
the remaining low-density escape is not merely that the interval-packet
density tends to zero pointwise: it must do so while still maintaining
...
* Lemma 8.5a.7u' adds the infinite-tail mass version: any cylinder family
...
* Corollary 16.86 packages robust core covers: if finitely many auxiliary
  intervals have robust cores covering the bounded active palette, then the
  corresponding interval packet cannot be dense.
* Corollary 16.87 connects the interval geometry to selector thinning:
  bounded-rank positive-density packet debt can product-cover every packet
