...
/-!
# Elementary Chebyshev bounds

This file proves explicit linear upper and lower bounds for Chebyshev's theta function.  The
lower bound uses the same central-binomial-coefficient estimates as the elementary proof of
Bertrand's postulate in mathlib.
-/
...
/-- Chebyshev's theta function, with the endpoint convention `p ≤ n`. -/
noncomputable def theta (n : ℕ) : ℝ :=
  ∑ p ∈ (Finset.range (n + 1)).filter Nat.Prime, Real.log p

/-- Theta is the logarithm of the primorial. -/
...
/-- A central binomial coefficient is bounded by a small-prime contribution times the primorial.
This is the factorization estimate underlying the lower Chebyshev bound. -/
theorem centralBinom_le_pow_sqrt_mul_primorial (n : ℕ) (hn : 0 < n) :
  ...
  · refine (Finset.prod_le_prod' fun p _ ↦ (?_ : f p ≤ 2 * n)).trans ?_
    ...
    · have hcard : ({p ∈ S | p ≤ Nat.sqrt (2 * n)} : Finset ℕ).card ≤ Nat.sqrt (2 * n) := by
        have hsubset : ({p ∈ S | p ≤ Nat.sqrt (2 * n)} : Finset ℕ) ⊆
            Finset.Icc 1 (Nat.sqrt (2 * n)) := by
          intro p hp
          simp only [Finset.mem_filter] at hp
          exact Finset.mem_Icc.mpr ⟨(Finset.mem_filter.mp hp.1).2.one_lt.le, hp.2⟩
        calc
          _ ≤ (Finset.Icc 1 (Nat.sqrt (2 * n))).card := Finset.card_le_card hsubset
          _ = Nat.sqrt (2 * n) := by rw [Nat.card_Icc, Nat.add_sub_cancel]
      simpa only [Finset.prod_const] using Nat.pow_le_pow_right (Nat.mul_pos Nat.two_pos hn) hcard
...
/-- An explicit exponential lower bound for primorials at even arguments. -/
theorem four_pow_two_mul_div_three_lt_primorial_two_mul (n : ℕ) (hn : 512 ≤ n) :
  ...
  have : 4 ^ n < n * ((2 * n) ^ Nat.sqrt (2 * n) * 4 ^ (2 * n / 3)) :=
    hfour.trans_le (Nat.mul_le_mul_left n (hchoose.trans (Nat.mul_le_mul_left _ hprim)))
  rw [← Nat.mul_assoc] at this
  exact (Nat.not_lt_of_ge hmain) this

/-- A convenient lower primorial bound valid at every sufficiently large argument. -/
...
/-- The explicit upper Chebyshev bound `theta(n) ≤ n log 4`. -/
...
/-- The explicit lower Chebyshev bound `(n / 4) log 4 ≤ theta(n)`. -/
...
/-- A genuinely linear lower bound, with no floor in the coefficient. -/
...
/-- Two-sided Chebyshev estimates, with explicit positive constants. -/
theorem theta_chebyshev :
    ∀ᶠ n : ℕ in Filter.atTop,
      (Real.log 4 / 5) * n ≤ theta n ∧ theta n ≤ Real.log 4 * n := by
  filter_upwards [Filter.eventually_ge_atTop 1024] with n hn
  constructor
  · exact theta_lower_linear n hn
  · simpa [mul_comm] using theta_le n

/-- The constants appearing in `theta_chebyshev` are positive. -/
theorem theta_chebyshev_constants_pos :
    0 < Real.log 4 / 5 ∧ 0 < Real.log 4 := by
  constructor
